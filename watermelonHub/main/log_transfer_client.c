...
void log_transfer_client_on_ctrl_notify(const uint8_t *data, uint16_t len)
{
	...
	case STATUS_OK: {
		...
		if (!g_state.fp) {
			...
			if (!g_state.buf) {
				ESP_LOGE(TAG, "RAM allocation failed for %lu bytes", (unsigned long)fileSize);
		...
		ESP_LOGI(TAG, "Transfer accepted; size=%lu bytes (dest='%s', RAM=%s)",
	...
	case STATUS_TRANSFER_DONE:
		...
		if (g_state.active) {
			ESP_LOGI(TAG,
			         "Transfer finished from shears: received=%lu bytes, expected=%lu",
			         (unsigned long)g_state.bytesReceived, (unsigned long)g_state.expectedSize);
...
static void dump_downloaded_file(void)
{
	...
	if (g_state.buf && g_state.expectedSize > 0) {
		ESP_LOGI(TAG, "Dumping first lines from RAM buffer (%lu bytes):",
		         (unsigned long)g_state.expectedSize);
